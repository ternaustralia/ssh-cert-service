...
# copy dist files into build container
...
RUN addgroup -g 1000 ssh_cert_service \
  && adduser -h /opt/venv -g 'ssh-cert-service' -G ssh_cert_service -D -H -u 1000 ssh_cert_service
...
# CMD ["gunicorn", "--bind=:5000", "--workers=2", "--threads=4", "--forwarded-allow-ips='*'",  "--statsd-host=statsd-exporter.services:9125", "--statsd-prefix=ecoimages_portal_api", "ssh_cert_service:create_app()"]

CMD ["gunicorn", "--bind=:5000", "--forwarded-allow-ips='*'",  "ssh_cert_service:create_app()"]
