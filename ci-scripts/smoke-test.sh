...
IMAGE_NAME=$1
CONTAINER_NAME=ssh_cert_service

# start up image
docker run -d -p 5000:5000 \
    --name $CONTAINER_NAME \
    -e SSH_CERT_SERVICE_OIDC_DISCOVERY_URL=http://localhost:5000/ \
    --add-host postgres:$BITBUCKET_DOCKER_HOST_INTERNAL \
    $IMAGE_NAME \
    gunicorn --bind :5000 --workers=1 --threads=2 'ssh_cert_service:create_app()'
